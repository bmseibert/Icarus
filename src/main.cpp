...
int main()
{
  ...
  if (calibrate){
    gpioServo(18, 0);
    gpioServo(18, 2000); //set it to max value
    std::this_thread::sleep_for(std::chrono::milliseconds(4000));
    gpioServo(18, 1000); // set it to the min value
    std::this_thread::sleep_for(std::chrono::milliseconds(12000));
    gpioServo(18, 0); // set it to 0
    std::this_thread::sleep_for(std::chrono::milliseconds(2000));
    gpioServo(18, 1000); // Now it is armed
    std::cout << "PWM set up done" << std::endl;
  }
  else{
    gpioServo(18, 0); //set it to 0
    std::this_thread::sleep_for(std::chrono::milliseconds(4000));
    gpioServo(18, 2000); // set it to the min value
    std::this_thread::sleep_for(std::chrono::milliseconds(4000));
    gpioServo(18, 1000); // set it to 0
    std::this_thread::sleep_for(std::chrono::milliseconds(4000));
  }
